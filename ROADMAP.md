...
- [x] Adopt the shared `zkm.pdftext` helper + run the density-ratio pilot (replaces the local min-text "scanned-only" heuristic) [HARD — decision gate] — 🚧 GATED (auto, id:3801; route:hard-split): DECOMPOSED into seams id:cd59, id:8aa4 — pick those, not this. Adoption half (id:cd59) SHIPPED 2026-06-24 (a60c76b). Pilot half (id:8aa4) remains data-gated: no zkm-pdf-skipped.jsonl corpus exists yet. — UMBRELLA CLOSED 2026-06-25 (/relay human): decision made 2026-06-18 + adoption seam id:cd59 shipped (a60c76b); remaining density-ratio pilot tracked as id:8aa4 ([HARD — meeting], data-gated on the skip-log corpus, left open). <!-- id:9475 -->
...
- [x] **[ROUTINE]** Adopt shared zkm.pdftext helper for scanned-only routing + shared pdf_text_threshold key (min_text_chars deprecated alias one release); remove plugin.yaml provisional NOTE; ARCHITECTURE routing-contract note; RED test pins routing on scanned-vs-text fixture. Mirrors shipped zkm-scan id:02bd. — seam of id:9475 (auto, id:3801) — DONE 2026-06-24 (a60c76b): convert.py imports `zkm.pdftext.resolve_threshold`; `pdf_text_threshold` canonical, `min_text_chars` deprecated alias; 2 RED→GREEN specs in test_roadmap_specs.py (verified discriminating against pre-cd59 impl); full suite 28 green. <!-- id:cd59 -->
- [ ] **[HARD — meeting]** Density-ratio pilot: per-page text-coverage discriminator calibrated from zkm-pdf-skipped.jsonl, report false-skip rate, fall back to char-count default if it does not beat it. — 🚧 GATED (data): the skip-log corpus must accumulate real `zkm-pdf-skipped.jsonl` entries before the pilot can be calibrated; not pickable until it exists. Coordinate with zkm-scan id:02bd. — seam of id:9475 (auto, id:3801) <!-- id:8aa4 -->
